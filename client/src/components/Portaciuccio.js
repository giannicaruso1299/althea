...
const Portaciuccio = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            await axios.post("//althea-bomboniere.it/api/items/portaciuccio")
                .then(res => {
                    setLoaded(true);
                    setItems(res.data);
                })
        ...
        document.title = 'Althea Bomboniere | Portaciuccio';
    ...
    return (
        <div className="container-fluid">
            <Affiliati/>
            <Header/>
            <h1 className="text-center mt-3">I nostri bellissimi portaciuccio</h1>
            {(!loaded) && (
